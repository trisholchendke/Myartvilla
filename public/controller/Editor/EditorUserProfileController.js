angular
.module('Editor')
.controller('EditorUserProfileController', ['$scope', '$rootScope', '$location','$window', '$http',EditorUserProfileController])

;
function EditorUserProfileController($scope,$rootScope,$location,$window,$http){
	...
	$scope.show_profile_pic = function(){
		window.location.href = '/Editor.html#/show_profile_pics';
	}
	...
	$scope.onworkupload = function(element) {
		 $scope.$apply(function($scope) {
              ...
              if(SelectedFile.type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || SelectedFile.type == 'application/pdf'){
            	 alert('its documents'); 
	...
	$scope.onVideoFileChosen = function(element) {
		 $scope.$apply(function($scope) {
               $scope.SelectedFile = element.files[0];
               SelectedFile= event.target.files[0];
               if(SelectedFile.type != 'image/pdf' && SelectedFile.type != 'image/jpeg'){
...
//				$(".DateFrom").val(data[0].dob);
	...
	$scope.modify_profile_details = function(coming_object){
		$.ajax({
	        type: 'Post',
	        url: '/modify_editor_user_profile',
	        dataType: 'json',
	        data:coming_object,
	        success: function (data) {
	        	if(data){
	        		$scope.$apply(function () {
	        			...
	        			$scope.sucess = "Success!";
	        			 setTimeout(function () {
	        			        window.location.href = '/Editor.html#/dashboard';
	        			 }, 2000);
	        		});
	        	}
	        }
