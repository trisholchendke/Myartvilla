angular
.module('ShortFilms')
.controller('ShortFilmsPostYourWorkController', ['$scope', '$rootScope', '$location',	'$window', '$http',ShortFilmsPostYourWorkController])
...
;
function ShortFilmsPostYourWorkController($scope,$rootScope,$location,$window,$http){
	$scope.countries = [
	                  "Afghanistan",
	                  "Åland Islands",
	                  "Albania",
...
//  				$scope.user.advertisement_type = SelectedFile.type;
	...
	$scope.post_your_work = function() {
			...
			Name = $scope.SelectedFile.name;
			 var exts = ['mp4','3gp','flv','webm','mkv','ogg','gif','avi','mov','wmv','mpg','mpeg'];
		      if (Name) {
	...
	$scope.onvideoUpload = function(){
		...
		$scope.post['post_role'] = "Short Films";
		$.ajax({
			type: 'Post',
			url: '/post_my_works',
			dataType: 'json',
			...
			success: function (data) {
				if(data){
					window.location.href = '/ShortFilms.html#/work_history';
				}
			}
		});
	};
