angular
.module('Publisher')
.controller('PublisherUserProfileController', ['$scope', '$rootScope', '$location','$window', '$http',PublisherUserProfileController])

;
function PublisherUserProfileController($scope,$rootScope,$location,$window,$http){
	...
	$scope.show_profile_pic = function(){
		window.location.href = '/Publisher.html#/show_profile_pics';
	}
		 ...
		 var exts = ['pdf','docx','txt','doc'];
	      if (Name) {
	        ...
	        if ( $.inArray ( get_ext[0].toLowerCase(), exts ) > -1 ){
	          ...
	          alert( 'Select only doc,pdf file.' );
	        }
	      }
	}
...
//				$(".DateFrom").val(data[0].dob);
	...
	$scope.modify_profile_details = function(coming_object){
		$.ajax({
	        type: 'Post',
	        url: '/modify_publisher_user_profile',
	        dataType: 'json',
	        data:coming_object,
	        success: function (data) {
	        	if(data){
	        		$scope.$apply(function () {
	        			...
	        			$scope.sucess = "Success!";
	        			 setTimeout(function () {
	        			        window.location.href = '/Publisher.html#/dashboard';
	        			 }, 2000);
	        		});
	        	}
	        }
