angular
.module('ShortFilms')
.controller('GiveawayThemeController', ['$scope', '$rootScope', '$location',	'$window', '$http',GiveawayThemeController])
...
;
function GiveawayThemeController($scope,$rootScope,$location,$window,$http){
//	alert('hi');
	$('#file-fr').fileinput({
        language: 'fr',
        uploadUrl: '#',
	...
	$scope.post_your_theme = function() {
			FReader = new FileReader();
			Name = $scope.SelectedFile.name;
			if($scope.post.file_type == 'image/jpeg' || $scope.post.file_type == 'image/png'){
...
//		alert('hi');
		    		...
		    		$scope.post['theme_id'] = Math.floor(new Date().getTime()/1000);
//		    		$scope.post['theme_url'] = 'http://localhost:8080/ShortFilms.html#/giveaways/+'$scope.post['theme_id'];
		            $.ajax({
		    	        type: 'Post',
		    	        url: '/post_your_theme',
		    	        dataType: 'json',
...
//		    	        		alert(JSON.stringify(data.theme_id));
		    	        		...
		    	        		window.location.href = 'ShortFilms.html#/set_question/' + theme_id;
//		    	        		location.href="/ShortFilms.html#/audience_quiz_details"
//		    	        		$location.path('/giveaways');
//		    	        		$location.path('/giveaways');
		    	        	}
		    	        }
		    		});
	...
	socket.on('Done', function(data) {
		$scope.post['theme_img'] = data['Image']
				.replace("public", "..");
