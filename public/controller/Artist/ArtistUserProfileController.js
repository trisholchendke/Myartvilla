angular
.module('Artist')
.controller('ArtistUserProfileController', ['$scope', '$rootScope', '$location',	'$window', '$http',ArtistUserProfileController])
.filter('capitalize',[capitalize])
;
function ArtistUserProfileController($scope,$rootScope,$location,$window,$http){
//	alert($window.sessionStorage.user_name);
	...
	$scope.show_profile_pic = function(){
		window.location.href = '/Artist.html#/show_profile_pics';
	}
	        	...
	        	if(data){
	        		$scope.$apply(function () {
	        			...
	        			$scope.sucess = "Success!";
	        			 setTimeout(function () {
	        			        window.location.href = '/Artist.html#/dashboard';
	        			 }, 2000);
	        		});
	        	}
	        }
