...
.module('Artist')
.run(['$rootScope',run])
.controller('ArtistDashboardController', ['$scope', '$rootScope', '$location','$window','$http','$timeout',ArtistDashboardController])
...
;
function ArtistDashboardController($scope,$rootScope,$location,$window,$http,$timeout){
...
//		  }
//	  $timeout(function(){
//		    $scope.url = 'http://127.0.0.1:8080/share_post.html#/';
//		    $scope.text = 'testing share';
//		    $scope.title = 'title1'
//		  },1000
//	  )
//	  $scope.share_google = function(coming_object){
		
//		  alert('hi');
//		  alert(JSON.stringify(coming_object.post_id));
//		  alert(JSON.stringify(coming_object.user_id));
//		  $window.localStorage.share_user_id = coming_object.user_id;
//		  $window.localStorage.share_post_id = coming_object.post_id;
//		  alert($window.localStorage.share_user_id)
//		  alert($window.localStorage.share_user_id)
//		  alert($window.localStorage.share_post_id)
//		   $.ajax({
//		  			type: 'Post',
//		  			url: '/get_share_post_data',
//		  			dataType: 'json',
//		  			data:{user_id:$window.localStorage.share_user_id,post_id:$window.localStorage.share_post_id},
//		  			success: function (data) {
//		  				$scope.$apply(function(){
...
//		  			}
//		   });
//		  javascript:window.open("https://plus.google.com/share?url=http://127.0.0.1:8080/share_post.html", '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return true;
//	  }
	 $scope.custom = true;
	...
	var refresh = function(){
		$.ajax({
	        ...
	        success: function (data) {
	        	...
	        	$scope.$apply(function () {
	                $scope.works = data;
//	                alert(JSON.stringify(data[0].post_owner[0].profile_pic,null,2));
	                $scope.profile_pic = data[0].post_owner[0].profile_pic;
//	                
//	               $scope.disabled = true;
//	                alert(JSON.stringify($scope.profile_pic));
	            });
	        }
		});
	}
